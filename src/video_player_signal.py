...
# this is deprecated in matplotlib3
    ...
    def run(self):
        ...
        while self.mw.isRun:
            ...
            if self.mw.isthreadActive:
                ...
                cnt = self.mw.frameID
                if (
                    cnt + 1 - self.mw.stframe < self.mw.ui.horizontalSlider.maximum()
                    and not self.mw.sliderbusy
                    and not self.mw.resizegoing
                ):

                    # self.mw.ui.horizontalSlider.setValue(cnt+1-self.mw.stframe)

                    pdraw = self.mw.drawmin
                    while (cnt + 1) / self.mw.fps > self.mw.data.values[
                        self.mw.drawmin, 1
                    ]:
                        self.mw.drawmin += 1
                    if not self.mw.drawmin == pdraw:
                        ...
                        self.mw.ax1.plot(
                            [
                                self.mw.data.values[self.mw.drawmin, 1],
                                self.mw.data.values[self.mw.drawmin, 1],
                            ],
                            [0, 3.4],
                            "k",
...
class workerThread(QThread):
    ...
    def run(self):
        ...
        while self.mw.isRun:
            ...
            if (
                self.mw.isthreadActive
                and self.mw.isbusy == False
                and self.mw.frameID != self.mw.cap.get(cv2.CAP_PROP_POS_FRAMES)
            ):
                # print(itr)

                if (
                    np.abs(self.mw.frameID - self.mw.cap.get(cv2.CAP_PROP_POS_FRAMES))
                    > 1
                ):
...
class MainForm(QMainWindow):
    ...
    def horizontalSliderIncrease(self, val):
        ...
        if self.ui.startButton.isEnabled():
            ...
            timg = cv2.resize(
                limg2,
                (
                    int(self.scaleFactor * limg2.shape[1]),
    ...
    def lineSliderSet(self, cnt):
        ...
        if not self.drawmin == pdraw or pdraw == 1:
            ...
            self.ax1.plot(
                [self.data.values[self.drawmin, 1], self.data.values[self.drawmin, 1]],
                [0, 3.4],
                "k",
                linewidth=2.0,
            )
    ...
    def horizontalSliderSet(self, cnt):
        if (
            cnt + 1 - self.stframe > self.ui.horizontalSlider.maximum()
            or self.sliderbusy
            or self.resizegoing
        ):
            return
    ...
    def draw(self):
        ...
        self.ax1.plot(
            [self.data.values[1, 1], self.data.values[1, 1]],
            [0, 3.4],
            "k",
            linewidth=2.0,
        )
